...
export class MemoryClient {
  ...
  /**
   * Poll a job until it completes (SUCCEEDED) or fails.
   * Used for Volcengine async mode where add() returns PENDING + event_id.
   */
  async checkJobStatus(
    ...
    while (Date.now() - start < timeout) {
      const result = await this._fetchWithErrorHandling(
        `${this.host}/v1/job/${eventId}/`,
        { headers: this.headers },
      );
      ...
      if (status === "FAILED") {
        throw new APIError(`Job failed: ${result?.error ?? "unknown error"}`);
      }

      await new Promise((r) => setTimeout(r, pollInterval));
    }

    throw new APIError(`Job ${eventId} timed out after ${timeout}ms`);
  }

  /**
   * Add memories and optionally wait for async completion.
   * If the response contains a PENDING result with event_id, polls until done.
   * For non-async APIs (standard Mem0 Platform), returns immediately.
   */
  async addAndWait(
    messages: Array<{ role: string; content: string }>,
    options: Record<string, any> = {},
    pollOpts: { pollInterval?: number; timeout?: number } = {},
  ): Promise<any> {
    const response = await this.add(messages, options);

    // Check if response indicates async processing (Volcengine)
    const results = response?.results;
    if (Array.isArray(results) && results.length > 0) {
      const first = results[0];
      if (first.status === "PENDING" && first.event_id) {
        const jobResult = await this.checkJobStatus(first.event_id, pollOpts);
        return jobResult;
      }
    }
